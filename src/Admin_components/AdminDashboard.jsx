...
const ContentArea = styled.div`
  flex: 1;
  padding: 20px;
  max-width: calc(100% - 320px); /* UserList와 ContentArea의 크기 합이 100% 되도록 설정 */
...
const UserBoxContainer = styled.div`
  ...
  @media (max-width: 600px) {
    grid-template-columns: repeat(2, 1fr); /* 화면 크기가 600px 이하일 때 2개씩 */
  }
`;
          ...
          {filteredUsers.length > 0 ? (
            ...
            {filteredUsers.map((user) => (
              <UserBox key={user.id}>
                ...
                <UserInfo>{user.gender} | {user.age}세</UserInfo>
              </UserBox>
            ))}
          </UserBoxContainer>
